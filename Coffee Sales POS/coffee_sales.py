...
class app:
    def __init__(self):
        """
        PURPOSE: initilize and run the coffee shop point-of-sales application\n
        ARGUMENT: None\n
        RETURN: None\n
        """
    ...
    def __get_customer(self) -> Union[Customer, Member]:
        """
        PURPOSE: initilize the customer or member object\n
        ARGUMENT: email: the customer's email\n
        RETURN: the customer or member object\n
        """
        while True:
            print("\n\nEnter your email to sign in or 'quit' to exit.")
            if (email := input("Email: ").strip().lower()) == "quit": self.__quit()
            elif not self.validator.is_customer(customer_record=(customer_record := self.database.get_customer(email=email))): continue 
            elif self.validator.is_member(customer_record=customer_record): return Member(customer_record=customer_record[0])
    ...
    def __get_menu_selection(self, menu_name: str, menu: dict) -> None:
        """
    ...
    def __search_products_by_attribute(self, attribute: str, attribute_value: str = None, shopping_cart: bool = False) -> list[Product]:
        ...
        """
        if attribute_value is None: attribute_value = input(f"Enter the {attribute.replace("_", " ").strip()} you would like to search for: ").strip().upper()
